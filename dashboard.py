...
# **Top 10 Most & Bottom 10 Least Purchased Categories**
...
# Plot kategori teratas
with col1:
    st.caption("Top 10 Most Ordered Categories")
    fig, ax = plt.subplots(figsize=(5, 5))
    sns.barplot(x="total_orders", y="product_category_name", data=top_categories, hue="product_category_name", dodge=False, palette="Blues_r", ax=ax)
    ax.set_xlabel("Total Orders", fontsize=10)
    ax.set_ylabel("Product Category", fontsize=10)
    wrap_labels(ax, width=15)
    st.pyplot(fig)

# Plot kategori terbawah
with col2:
    st.caption("Bottom 10 Least Ordered Categories")
    fig, ax = plt.subplots(figsize=(5, 5))
    sns.barplot(x="total_orders", y="product_category_name", data=bottom_categories, hue="product_category_name", dodge=False, palette="Reds_r", ax=ax)
    ax.set_xlabel("Total Orders", fontsize=10)
    ax.set_ylabel("Product Category", fontsize=10)
    wrap_labels(ax, width=15)
    st.pyplot(fig)
...
sns.barplot(data=payment_counts, x="payment_type", y="percentage", hue="payment_type", dodge=False, palette="Reds_r", ax=ax)
...
# **Top Number Transaction per State**
...
sns.barplot(x="customer_state", y="total_orders", data=state_transaction_counts, hue="customer_state", dodge=False, palette="Blues_r", ax=ax)
...
with tab1:
    ...
    sns.barplot(y=top_recency['customer_id'].astype(str), x=top_recency['recency'], hue=top_recency['customer_id'].astype(str), palette='Blues', legend=False, ax=ax)
